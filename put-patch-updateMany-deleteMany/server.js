...
app.post("/login",async(req,res)=>{
   console.log(req.body);
 let result =  User.find().and({ email: req.body.email });
...
app.put("/updateDetails",upload.single("profilePic"),async(req,res)=>{
   ...
   try {
    if(req.body.firstName.trim().length>0){
        await updateMany({email:req.body.email},{firstName:req.body.firstName});
    }
    if(req.body.lastName.trim().length>0){
        await updateMany({email:req.body.email},{lastName:req.body.lastName});
    }
    if(req.body.age>0&&req.body.age<100){
        await updateMany({email:req.body.email},{age:req.body.age});
    
    }
    if(req.body.password.trim().length>0){
    await updateMany({email:req.body.email},{password:req.body.password})
    }
    if(req.body.mobileNo.length>0){
     await updateMany({email:req.body.email},{mobileNo:req.body.mobileNo});
    }
    if(req.file&&req.file.path){
    await updateMany({email:req.body.email},{profilePic:req.file.path})
    }
